...
```JavaScript
var book = new ABCXJS.TuneBook(tunebook);
...
IMPORTANT: Version 1.1 has removed all globals and any side effects of ABCXJS except for this single global:
...
```
New name			Old name
ABCXJS.numberOfTunes	numberOfTunes
ABCXJS.renderAbc		renderABC
ABCXJS.renderMidi	renderMidi
ABCXJS.Editor		ABCEditor
ABCXJS.plugin		abc_plugin
```
...
<script type="text/javascript">
        window.onload = function() {
                abc_editor = new ABCXJS.Editor("abc", { canvas_id: "canvas0", midi_id:"midi", warnings_id:"warnings" });
        }
</script>
```

The ABCXJS.Editor constructor takes the following parameters:
...
```html
<script type="text/javascript">
  ABCXJS.plugin["show_midi"] = false;
  ABCXJS.plugin["hide_abc"] = true;
</script>
```
...
(To use the ABCXJS.parse.Parser and ABCXJS.write.Writer constructors independently of the abcjs editor and abcjs plugin, see how they are used in abcjs editor.)

There are these functions that make it easy to render the sheet music and the midi:

```javascript
ABCXJS.numberOfTunes(abcString);
ABCXJS.renderAbc(div, abcString, parserParams, printerParams, renderParams);
ABCXJS.renderMidi(div, abcString, parserParams, midiParams, renderParams);
```
