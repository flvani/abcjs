...
ABCXJS.renderAbc = function(output, abc, parserParams, printerParams, renderParams) {
	function callback(div, tune) {
		...
		if (printerParams === undefined)
			printerParams = {};
		var printer = new ABCXJS.write.Printer(paper, printerParams);
...
//			startingTune: an index, starting at zero, representing which tune to start rendering at.
//				(If this element is not present, then rendering starts at zero.)
ABCXJS.renderMidi = function(output, abc, parserParams, midiParams, renderParams) {
	function callback(div, tune) {
		if (midiParams === undefined)
			midiParams = {};
               if (ABCXJS.midi && ABCXJS.midi.MidiWriter && typeof div !== "undefined" ) {
  		  var midiwriter = new ABCXJS.midi.MidiWriter(div, midiParams);
